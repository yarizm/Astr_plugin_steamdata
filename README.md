...
插件目录为 `astrbot_plugin_steamdata/`。
...
如果未来插件代码改用更高版本 AstrBot API，请同步调整 `astrbot_plugin_steamdata/metadata.yaml` 中的 `astrbot_version`。
...
```text
data/plugins/Astr_plugin_steamdata/astrbot_plugin_steamdata/
```
...
python -m pip install -r astrbot_plugin_steamdata/requirements.txt
```
...
插件配置文件由 AstrBot 根据 `astrbot_plugin_steamdata/_conf_schema.json` 生成。Steam Web API Key 可以在 [Steam Web API Key 页面](https://steamcommunity.com/dev/apikey) 申请。
...
```bash
python -m py_compile astrbot_plugin_steamdata/main.py
